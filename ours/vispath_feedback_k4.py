...
class VisPathFeedback:
    def __init__(self, api_key=None, model="gpt-4o-mini", system_prompt_expansion=None, system_prompt_codegen=None, template_expansion=None, template_codegen=None, system_prompt_aggregation=None, template_aggregation=None, temperature=0.2):
        ...
        self.system_prompt_expansion=system_prompt_expansion or "You are an expert on extend data visualization query. You should think step by step, and write the extended queries in the format of ```[query_text_1, query_text_2]```"

        self.template_expansion=template_expansion or """\
                Think step by step. Generate three distinct extended queries based on the given query. Each query should emphasize a different aspect or perspective of the original query. Present the queries in the following format: ```[query_text_1, query_text_2]```, where query_text_n refers to the generated queries.
